...
void saveCentersToSSV(Eigen::VectorXd pos, int N, int i, int nT)
{
	FILE* out;
	stringstream ss;
	static int number = 0;
	ss << "NCells_" << N << "_number_" << i << "Nt_" << nT;
...
void run(int N, dbl phi, int seed, int iter, int nT, int test_num=1)
{
	//Create the system
	...
	for (int i = 0; i < N; i++)
	{
		for (int d = 0; d < DIM; d++)
		{
			positions(DIM * i + d) = ((double) rand()) / RAND_MAX;
		}
	}

	s.SetPositionsVirtual(positions);
	s.SetRadiiPolyUniform(3.0);
	s.SetPackingFraction(0.9);

	//Minimize the energy:
	CStaticComputer<DIM> c(s);
	CSimpleMinimizer<DIM> miner(c, CSimpleMinimizer<DIM>::FIRE); //Note: the miner class should be able to create the Computer (i.e. just pass it the state)

	Eigen::VectorXd pos;
	s.GetPositionsVirtual(pos);
	
	saveCentersToSSV(pos, N, iter, nT);
...
int main(int argc, char* argv[])
{
	...
	int c;
	while((c=getopt(argc, argv, "n:f:r:t:i:a:")) != -1)
		switch(c)
		{
...
#pragma omp for
	...
	*/
	...
	run(N, phi, r + 9, iter, nT, test_num);
