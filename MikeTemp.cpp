...
void writePsi6Data(char filename[], vector< vector<double> > &psi6p, int rec)
{
	char fn[500];
	sprintf(fn, "%sPsi6Prec_%d", filename, rec);
...
void calculatePsi6P(char filename[])
{
	char* fn = (char *) malloc(200);
	char* xS = (char *) malloc(100);
	char* yS = (char *) malloc(100);
	char* fileNameOrig  = (char *) malloc(500);
	sprintf(fileNameOrig, "%s.txt", filename);
	cout << fileNameOrig << endl;
	ifstream in(fileNameOrig);
	double x, y;
	...
	{
		char fntemp[500];
		sprintf(fntemp, "MinimizedState%s", fileNameOrig);
		cout << fntemp << endl << endl;
		CStaticDatabase<DIM> state_db(N, fntemp, NcFile::ReadOnly);
		for (int rec = 0; rec < state_db.GetNumRecs(); rec++)
		{
			CStaticState<DIM> state(N);
			vector< vector<double> > psi6p;
			state_db.Read(state, rec);
			CStaticComputer<DIM> computer(state);
			CSimpleMinimizer<DIM> miner(computer, CSimpleMinimizer<DIM>::FIRE);
			computer.StdPrepareSystem();
			computer.CalculatePsi6Mag(psi6p);
			writePsi6Data(fntemp, psi6p, rec);
			vector< vector<int> > neighbors;
			CBondList<DIM> bonds;

			/*
			computer.ComputeBondList(bonds);
			bonds.CalculateNeighbors(neighbors);
